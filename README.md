...
**Student Name  :** S.Shushmitha  
**Student ID    :** 2310040063  
...
```
[The `fitness()` function returns the total value of the selected items if the total weight is within the knapsack capacity. If the total weight exceeds the allowed capacity, it returns 0.

An overweight solution scores 0 because it violates the problem constraint, making it an invalid solution. This penalty ensures the algorithm avoids selecting infeasible solutions. ]
```
...
Higher-fitness individuals are more likely to be chosen because they have a better chance of winning these tournaments, which biases selection toward stronger solutions while still maintaining some diversity. ]
```
...
```
[ This line copies the best chromosome from the current generation into the next generation unchanged. This technique is called elitism.

It is important because it ensures that the best solution found so far is not lost due to crossover or mutation, helping the algorithm steadily improve or at least retain the best result. ]
```
...
|--------------|-----------------|-------------|--------|----------------|
| 0.01         |      75         |    14.4     |   YES     |   Rapid rise then early plateau             |
| 0.05         |      77         |    14.9     |   YES    |     Smooth rise then stable convergence           |
